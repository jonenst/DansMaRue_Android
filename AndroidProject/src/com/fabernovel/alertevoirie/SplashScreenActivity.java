	...
	private void init() {
		...
		try {
			SharedPreferences sp = getSharedPreferences(
					JsonData.PARAM_CATEGORIES, Context.MODE_PRIVATE);
			JSONObject categoriesRequest = new JSONObject().put(
					JsonData.PARAM_REQUEST,
					JsonData.VALUE_REQUEST_GET_CATEGORIES).put(
					JsonData.PARAM_CURRENT_VERSION,
					sp.getString(JsonData.PARAM_CURRENT_VERSION, "0"));

			AVService.getInstance(this).postJSON(
					new JSONArray().put(categoriesRequest), this);
	...
	public void onRequestcompleted(int requestCode, Object result) {
		if (requestCode == AVService.REQUEST_JSON && result != null) {
			try {
				...
				JSONObject answerObj = (JSONObject) rootObj
						.get(JsonData.PARAM_ANSWER);
				...
				if (!sp.getString(JsonData.PARAM_CURRENT_VERSION, "0").equals(
						version)) {
					...
					// Categories
					JSONObject categories = (JSONObject) answerObj
							.get(JsonData.PARAM_CATEGORIES);
					getApplicationContext().deleteFile("categories.json");
					try {
						FileOutputStream fos = getApplicationContext()
								.openFileOutput("categories.json",
				...
				new AlertDialog.Builder(SplashScreenActivity.this)
						.setTitle(R.string.error_popup_title)
